window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function profileModels(profile) {
			...
			for (const row of rows) {
				...
				const contextWindow = typeof row["contextWindow"] === "number" && Number.isSafeInteger(row["contextWindow"]) && row["contextWindow"] > 0 ? row["contextWindow"] : void 0;
				models.push({
					id,
					name,
					contextWindow
		...
		*/
		function modelBaselines(entry) {
			const baselines = /* @__PURE__ */ new Map();
			const { defaultContextWindow } = asProfile(entry.profile);
			const fallback = typeof defaultContextWindow === "number" && defaultContextWindow > 0 ? defaultContextWindow : void 0;
			...
			if (rows !== void 0) {
				for (const row of rows) {
					...
					if (value !== void 0) baselines.set(id, value);
				}
				return baselines;
			}
			...
			for (const model of entry.discovered) {
				...
				const value = typeof patched === "number" && patched > 0 ? patched : ceilings.get(model.id) ?? fallback;
				if (value !== void 0) baselines.set(model.id, value);
			}
			return baselines;
		}
		/**
		...
		*/
		function planRouteWithModels(entry, modelWindows) {
			...
			const capFor = (id) => ceilings.get(id) ?? 1024e3;
			if (rows === void 0) {
				...
				const nextRows = rows.map((row) => {
					const id = typeof row["id"] === "string" ? row["id"] : void 0;
					const target = id === void 0 ? void 0 : modelWindows.get(id);
					if (target === void 0) return row;
					const writeValue = Math.min(target, id === void 0 ? 1024e3 : capFor(id));
					if (id === void 0 || writeValue === row["contextWindow"]) return row;
					return {
						...row,
						contextWindow: writeValue
					};
				});
				if (nextRows.some((row, index) => row !== rows[index])) ops.push({
					op: "set",
					path: at("models"),
					value: nextRows
				});
		...
		*/
		...
		var Section_module_css_default = {
			...
			"routeHeadRoot": "_26wL1W_routeHeadRoot",
		...
		function RouteRow(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					...
					className: Section_module_css_default.routeHeadRoot,
					rowClassName: Section_module_css_default.routeHead,
